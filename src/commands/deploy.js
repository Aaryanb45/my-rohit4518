...
function generateK8sDeployment(serviceName, dockerImage, serviceType) {
  const port = serviceType === 'frontend' ? 80 : serviceType === 'database' ? 5432 : 8002;
...
async function deployToEKS(serviceName, service, clusterName, region) {
  ...
  try {
    ...
    const deploymentYaml = generateK8sDeployment(serviceName, service.docker_image, service.service_type);
...
const deployCommand = new Command('deploy')
  ...
  .option('-c, --cluster <name>', 'EKS cluster name', 'arnav_velora')
